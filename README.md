...
对标 al-folio / academicpages / HugoBlox 的栏目结构（Publications · Projects · Talks · People）
以及多份实验室网站规范里反复强调的「Projects / Grants 对合作与招生很关键」，
补上我们缺的两个板块，并给长文加了目录。

### 新增「科研项目」板块

之前成果页虽然能填「项目」类型，但项目的关键信息（资助机构、编号、起止时间、承担角色）
在论文条目里根本显示不出来。现在单独成板块：

- **页面**：`/projects/`，按起始年份倒序分组，卡片展示类别 / 状态 / 承担角色 / 起止时间 /
  资助机构 / 项目编号 / 经费 / 负责人 / 参与成员，正文支持 Markdown。
- **参与成员与负责人会链接到成员页**（姓名与「团队成员」里的姓名一致时自动匹配）。
- **后台**：「科研项目」模型，类别与状态用下拉建议（可自定义），含 `0008` 迁移。
- **导航**：新增「科研项目」「加入我们」两项，老站点已保存的导航会被自动补上这两项
  （插在「联系我们」之前），不用手动改数据库。

### 新增独立的「加入我们」页面

招生/招募是科研站最关键的转化页，之前只作为「联系我们」里的一个区块，导航上看不到。现在：

- **页面**：`/join/`，左侧是招生说明与 Markdown 详情（我们在找什么样的人 / 申请材料 /
  申请流程 / 常见问题），右侧是「我们正在做的方向」与「怎么联系我们」两张卡片。
- 首页招聘条与联系页都会指向这一页；后台里「招生信息」的字段不变，只是多了一个展示入口。
- 若后台关闭了招生信息，页面仍会给出联系入口，不会变成死路。

### 长文目录（TOC）

研究方向、新闻详情页的正文小节达到 **3 个以上**时，自动在正文前显示目录，
标题会自动生成锚点（中文标题直接作为 id，如 `#主要研究内容`），并加了
`scroll-margin-top`，跳转后不会被吸顶顶栏挡住。逻辑在 `src/utils/toc.mjs`，有单元测试。

## 前端补全（2026-09-14 · 第四轮）

### 成员 ↔ 成果 双向关联

之前成员个人页看不到他/她发表过什么，成果条目里的作者名也不能点。现在打通了：

- **成员个人页新增「参与论文」**，按年份从新到旧列出该成员参与的全部成果。
- **成果条目里的作者名可点**：作者的署名与成员的英文名（或中文名）一致时，自动链到成员页，
  用虚线下划线标注「可点」，鼠标悬停才变实线。外部合作者仍是纯文本。
- **成果页新增「作者」筛选**，下拉项只列出确实有论文的成员，可以一键看某位同学发表过什么。

匹配规则放在 `src/utils/authors.mjs`（有单元测试）：**归一化后精确相等**，
不做模糊匹配 —— 中文姓名只有两三个字，模糊匹配很容易把不相干的人算进来。

> 后台录入时注意：成果的「作者列表」要写成员的英文名（与「团队成员」里的英文名一致），
> 关联才会生效。填「Zhang Wei」而成员写的是「Wei Zhang」不会匹配上。

### 其他

- **封面图不再让布局抖动**：研究方向详情页与新闻详情页的封面改为按 16:9 占位后再加载图片，
  正文不会被图片加载顶下去。这两处本来就是「封面」而不是正文插图，按比例裁切是常规做法。
- **首页栏目条数可在后台调**：新增「成果栏目显示条数」「近况栏目显示条数」两个文案键，
  默认 4，取值范围自动收敛到 1–12，填错也不会把首页撑爆。
- **作者之间的分隔符可配置**：中英文署名习惯不同（`，` 与 `, `），后台可改。

## 后台内容地图（2026-09-14 · 第三轮）

目标：**前台能看到的每一处内容，管理员都能在后台改。** 后台入口是 `/api/admin/`。

| 前台位置 | 后台在哪里改 |
| --- | --- |
| 首页主视觉（标题、英文副标题、说明、背景图、按钮） | **首页轮播**：只使用排序最前的已发布项 |
| 课题组名称、英文名、缩写、一句话简介、所属单位、搜索引擎简介 | **站点设置 → 课题组信息** |
| 首页「关于我们」的团队合照 | **站点设置 → 课题组信息 → 首页团队合照** |
| 通讯地址、邮编、邮箱、电话 | **站点设置 → 联系方式** |
| 招生标题 / 说明 / 详情（申请材料、流程、常见问题）/ 接收邮箱 | **站点设置 → 招生信息** |
| 导航菜单、页脚链接、备案号 | **站点设置 → 导航与页脚** |
| 各页面的标题、介绍、按钮文字、空状态、筛选器与提示语（共 162 条） | **站点设置 → 各页面文案**（首页 / 研究方向页 / 成员页 / 新闻页 / 成果页 / 联系页 / 科研平台页 / 科研项目页 / 加入我们页 / 站内搜索 / 页面提示 / 页脚 十二组，点开折叠块即可改） |
| 成员页的身份分组顺序、是否出现「其他成员」 | **站点设置 → 成员页文案 → 身份分组顺序** |
| 研究方向（标题、简介、图标、关键词、配图、正文、排序、发布） | **研究方向** |
| 团队成员（姓名、身份、去向、研究方向、照片、链接、简介……） | **团队成员** |
| 科研新闻（标题、日期、摘要、标签、封面、置顶、正文） | **科研新闻** |
| 科研成果（标题、作者、期刊会议、类型、方向、精选、链接、摘要） | **科研成果** |
| 科研项目 / 基金（名称、类别、资助机构、编号、角色、负责人、参与成员、起止年份、状态、经费） | **科研项目** |
| 科研平台的机器人项目 | **科研平台·机器人项目** |
| 后台里上传过的图片 | **图片文件**（随内容自动产生，一般不用手动管） |

保存内容只代表数据已经写入数据库；线上静态页面还要成功重建并完成部署后才会更新。默认会在保存后自动请求重建，关闭自动重建时请手动执行「🚀 重建前台站点」。

**边界（有意不放进后台的）**：纯无障碍用的文字，例如「跳到主要内容」「切换深色模式」「上一张 / 下一张」这类屏幕阅读器标签，仍写在代码里。它们不是给访客看的内容，放进后台反而容易被改坏。

## 后台完整性由测试保证

「后台能不能改到某个字段」不该靠人工核对。`api/core/tests.py` 里的 `AdminCoverageTests` 会在每次测试时自动检查：

- **每个模型的可编辑字段都出现在后台分组里**（历史上漏加 `fieldsets` 会导致字段在后台根本不出现，而且不报错）；
- **`page_copy.json` 里每一条文案都有对应的后台输入框**；
- 没有出现意料之外的空白默认值；
- 首页轮播可增删改、并通过接口交给前端；
- 成员身份可以填自定义值（例如「访问学者」）。

新增字段或新增文案键时，如果忘记同步到后台，测试会直接失败并列出漏掉的字段名。
...
工作台会提示自动发布是否开启、发布服务是否配置。发布过程要区分三个状态：

1. **已保存到数据库**：后台保存成功，公开接口可以读到新内容，但线上静态页面可能仍是旧版。
2. **重建请求已接受**：后台收到发布服务的成功响应，只说明重建已经进入部署流程，不代表构建成功。
3. **部署已就绪**：部署平台显示构建成功，生产域名已切换到新部署，此时线上页面才完成更新。

因此，后台提示重建请求成功后，仍需到部署平台确认状态为 Ready（就绪）。
...
**默认情况下，保存内容后会自动请求一次前端重建**。请求被接受后通常还需约 1 分钟完成构建与部署；只有部署平台显示 Ready，网站才算更新完成。
