...
| 研究方向 | 带图标、关键词、排序、Markdown 正文 |
| 团队成员 | 身份分组（导师/博士后/博士生/硕士生/本科生/科研助理/校友）、照片、链接 |
| 科研新闻 | 日期、标签、置顶、Markdown 正文；支持「用 AI 根据正文生成摘要」 |
| 科研成果 | 作者、期刊会议、年份、类型、所属方向、精选、原文/PDF/代码链接 |

### 两个关键动作

1. **改完内容 → 勾选条目 → 执行「🚀 重建前台站点」**
   这一步会调用 Vercel Deploy Hook，让前端重新构建。
   *不执行这一步，改动只存在数据库里，网站上还看不到。*
2. **「用 AI 根据正文生成摘要」**
   需要配置 `DEEPSEEK_API_KEY`，没配也不影响其它功能。
...
**Q：前台内容没跟着后台更新？**
后台改完必须执行「🚀 重建前台站点」动作。若已执行，去 Vercel → Deployments 看最新一次构建是否成功。
